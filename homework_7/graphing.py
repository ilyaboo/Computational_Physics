...
def parse_results_2(filename: str, total_time: int) -> list[tuple[list[float], list[float], list[float], list[float], list[float]]]:
...
def graph_Es(folder_path: str, num_steps: list[int], total_time: int, title: str):
    ...
    for step_num in num_steps:
        ...
        plt.plot(times, Es, "--", label = "dt = " + str(dt) + " s")
...
def plot_Ps_Es_errors(data: tuple[list[float], list[float], list[float], list[float], list[float]], N: int, reps: int):
    ...
    ending = "for system of size N = " + str(N) + ", velocity of v = " + str(1 / bin_times[-1]) + "\nand number of repetitions of reps = " + str(reps)
